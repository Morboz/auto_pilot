...
class ToolExecutionLog(SQLModel, table=True):
    ...
    task_id: Optional[UUID] = Field(default=None, index=True, description="关联的 Task ID (可选)")
    tool_id: Optional[UUID] = Field(default=None, description="调用的 Tool ID (可选)")
    tool_name: str = Field(index=True, description="工具名称")
    input_params: Optional[str] = Field(default=None, description="调用参数（JSON 字符串）")
    output: Optional[str] = Field(default=None, description="工具返回结果（JSON 字符串）")
    error_message: Optional[str] = Field(default=None, description="错误信息（如工具调用失败）")
    ...
    resource_usage: Optional[str] = Field(default=None, description="资源使用情况（JSON 字符串）")
